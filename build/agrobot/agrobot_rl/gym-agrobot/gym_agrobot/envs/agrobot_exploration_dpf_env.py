...
class AgrobotExploreEnv(gym.Env):
    ...
    def get_reward(self):
        ...
        old = np.sum(np.abs(self.old_state - 127)/127)
        new = np.sum(np.abs(self.state - 127)/127)
        map_reward = np.clip((new - old) / self.max_change, 0, None)
        ...
        total_reward = fruit_reward + map_reward + contour_reward - self.step_dicount
        print(f'Total: {total_reward}, Map reward: {map_reward}, old: {old}, new: {new}, contour: {contour_reward}')
        return total_reward
...
def main():
    try:
        gym.envs.register(id="agrobot-v1", entry_point="gym_agrobot.envs:AgrobotExploreEnv")
        env = gym.make("agrobot-v1")
        model = PPO('CnnPolicy', env, learning_rate=1e-3, verbose=1)
        # check_env(env)
